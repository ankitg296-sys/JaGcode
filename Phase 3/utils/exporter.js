...
function exportPDF(candidates, query, callback) {
  ...
  candidates.forEach((c, i) => {
    ...
    doc.fillColor(verdictColor(c.verdict)).fontSize(9).font("Helvetica-Bold").text(c.verdict || "", 92, cardY + 42);
    ...
    if (c.skills?.length) {
      doc.fillColor(COLORS.muted).fontSize(8).font("Helvetica").text(c.skills.slice(0, 8).join("  ·  "), 62, y + 4, { width: cardWidth - 20 });
      y += 18;
    }

    const contact = [c.email, c.phone].filter(Boolean).join("   ");
    if (contact) doc.fillColor(COLORS.accent).fontSize(8).font("Helvetica").text(contact, 62, y + 4);
