...
function read(key) {
  ensureDataDir();
  const p = PATHS[key];
  if (!fs.existsSync(p)) return key === "candidates" ? {} : [];
  try { return JSON.parse(fs.readFileSync(p, "utf8")); } catch { return key === "candidates" ? {} : []; }
}
...
function saveSavedSearch(search) {
  ...
  if (idx >= 0) searches[idx] = search;
  else searches.unshift(search);
  write("savedSearches", searches);
}

function deleteSavedSearch(id) {
  const searches = read("savedSearches").filter(s => s.id !== id);
  write("savedSearches", searches);
}
...
function deleteJD(id) {
  const jds = read("jds").filter(j => j.id !== id);
  write("jds", jds);
}
